...
class Utils():
    ...
    def objecto_to_json(obj):
        dict_obj = Utils.para_dict(obj)
        return json.dumps(dict_obj)

    def string_to_objeto_item(string):
        itens = []
        list_dict = json.loads(string)
        for item in list_dict:
            obj_item = ItemModel(produto=item["produto"], preco=item["preco"], qtd=item["qtd"])
            itens.append(obj_item)
        return itens
